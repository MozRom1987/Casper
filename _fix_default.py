...
<head>
    ...
    <style>
        ...
        {{#get "tags" limit="all"}}
        {{#foreach tags}}
        .ctag-{{slug}} { --ctag-color: {{#if accent_color}}{{accent_color}}{{else}}var(--main-color){{/if}}; }
        .ctag-{{slug}} svg { fill: {{#if accent_color}}{{accent_color}}{{else}}var(--main-color){{/if}} !important; }
        .ctag-{{slug}} .epcl-category-overlay { background: {{#if accent_color}}{{accent_color}}{{else}}#fff{{/if}}; }
        .ctag-{{slug}} .epcl-category-overlay:before { background: {{#if accent_color}}{{accent_color}}{{else}}var(--main-color){{/if}}; }
        .main-article.primary-tag-{{slug}} .epcl-category-overlay { background: {{#if accent_color}}{{accent_color}}{{else}}#fff{{/if}}; }
        .main-article.primary-tag-{{slug}} .epcl-category-overlay:before { background: {{#if accent_color}}{{accent_color}}{{else}}var(--main-color){{/if}}; }
        {{/foreach}}
        {{/get}}
...
<body class="{{body_class}}">
    ...
    <nav class="mobile main-nav hide-on-desktop-lg">
        <ul class="menu">
            ...
            <li class="nav-{{slug}}{{#if current}} current-menu-item{{/if}}"><a href="{{url}}">{{label}}</a></li>
            {{/foreach}}
            <li class="nav-login">
                {{#unless @member}}
                <a href="#/portal/signup" class="login epcl-button dark" data-title="Sign Up">Sign Up</a>
                {{else}}
                <a href="#/portal/account" class="login epcl-button dark" data-title="Account">Account</a>
                {{/unless}}
            </li>
        </ul>
    </nav>
                ...
                <div class="grid-container">
                    <div class="bg-box">
                        ...
                        <div class="epcl-social-buttons fill-color hide-on-tablet hide-on-mobile hide-on-desktop-sm">
                            {{#if @site.facebook}}<a href="https://www.facebook.com/{{@site.facebook}}" class="facebook" target="_blank" rel="nofollow noopener"><i class="fa fa-facebook"></i></a>{{/if}}
                            {{#if @site.twitter}}<a href="https://twitter.com/{{@site.twitter}}" class="twitter" target="_blank" rel="nofollow noopener"><svg><use xlink:href="#twitter-icon"></use></svg></a>{{/if}}
                            <a href="{{@site.url}}/rss/" class="rss" target="_blank"><i class="fa fa-rss"></i></a>
                        </div>
                        {{/if}}
        ...
        <footer id="footer">
            ...
            <div class="widgets grid-container grid-large hide-on-mobile">
                <div class="epcl-row">
                    ...
                    {{!-- About widget --}}
                    <section class="widget widget_text bg-box grid-33 tablet-grid-50 mobile-grid-100">
                        <div class="logo">
                            <a href="{{@site.url}}">
                                {{#if @site.logo}}
                                <img src="{{@site.logo}}" alt="{{@site.title}}" width="170" />
                                {{else}}
                                <span class="title">{{@site.title}}</span>
                                {{/if}}
                            </a>
                    ...
                    <div class="widget widget_epcl_posts_thumbs bg-box grid-33 tablet-grid-50 mobile-grid-100">
                        <h4 class="widget-title title medium bordered">Latest Articles</h4>
                        {{#get "posts" limit="4"}}
                        {{#foreach posts}}
                        <article class="item ctag-{{primary_tag.slug}}">
                            <div class="info">
                                {{#if feature_image}}
                    ...
                    {{!-- Tags widget --}}
                    <div class="widget bg-box grid-33 tablet-grid-50 mobile-grid-100">
                        <h4 class="widget-title title medium bordered">Categories</h4>
                        {{#get "tags" limit="10"}}
                        <div class="widget_tag_cloud">
                            {{#foreach tags}}
                            <a href="{{url}}" class="ctag ctag-{{slug}}" data-id="ctag-{{slug}}">
                                <svg><use xlink:href="#tag-decoration"></use></svg>{{name}}
                            </a>
                            {{/foreach}}
                        </div>
                        {{/get}}
            ...
            {{!-- Copyright --}}
            <div class="published textcenter" style="padding: 20px 0;">
                <p>&copy; {{date format="YYYY"}} <a href="{{@site.url}}">{{@site.title}}</a>. Powered by <a href="https://ghost.org" target="_blank">Ghost</a>.</p>
            </div>

            {{!-- Secondary navigation --}}
            {{#if @site.secondary_navigation}}
            <nav class="secondary-nav textcenter" style="padding-bottom: 20px;">
                <ul>
                    {{#foreach @site.secondary_navigation}}
                    <li><span class="sep"></span><a href="{{url}}">{{label}}</a></li>
                    {{/foreach}}
                </ul>
            </nav>
            {{/if}}
